...
| FE-001 | Scaffold web app shell | Nova | P1 | review | Search/results shell now follows UX-001 and now includes a web API adapter/repository layer with server-driven search + detail loading, slug/id route helpers, safe mock fallback while API wiring is incomplete, and a minimal Vitest harness covering Google resolve redirect + cache-miss warning state |
| OPS-001 | Set up CI, branch protections, deployment environments | Forge | P1 | todo | After initial scaffold |
| OPS-002 | Make local MVP stack runnable via Compose | Forge | P1 | review | Added `web + api + postgres` Compose flow, env examples, safe repeatable API bootstrap, and fixed API UUID place slug parsing in web detail routes |
