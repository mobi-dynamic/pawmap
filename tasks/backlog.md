...
| BE-002 | Scaffold API foundation for search, place detail, reports, and moderation | Raven | P0 | review | FastAPI scaffold now exposes integration-ready search/detail payloads for frontend wiring, including realistic seeded places, explicit unknown-policy detail state, and Google resolve/cache-miss contract |
| FE-001 | Scaffold web app shell | Nova | P1 | review | Search/results shell now follows UX-001 and now includes a web API adapter/repository layer with server-driven search + detail loading, slug/id route helpers, and safe mock fallback while API wiring is incomplete |
