...
| BE-004 | Add Google ingestion + canonical upsert path for geography seeding | Raven | P0 | review | Added deterministic Google normalization/upsert path, DB CLI ingest command, sample Fitzroy seed file, and tests |
| BE-005 | Add bounded Google collection adapter for launch geography seeding | Raven | P0 | review | Added launch-geo collection planning, fixture/live Google collector modes, `collect-google` CLI, reusable snapshot output, and tests that feed the BE-004 ingestion path |
...
| FE-003 | Add admin moderation UI and review actions | Nova | P1 | review | Added `/admin/moderation`, typed admin moderation repository functions, approve/reject route handlers, env-backed local admin headers, and repository coverage; verified with test + typecheck + build |
| FE-004 | Add authenticated report submission UI on place detail | Nova | P1 | review | Added place-detail user report form, typed submission repository + route, env-backed local dev user header forwarding, focused repository coverage, and passed `npm test`, `npm run typecheck`, `npm run build` locally |
...
| OPS-003 | Switch deployment baseline to Netlify + Render + Neon | Forge | P1 | review | Vercel-first assumption removed from docs; added deployment guide for the new free-tier baseline; follow-up docs now clarify the failing `Vercel` PR status is an external integration blocker rather than a repo-side failure |
